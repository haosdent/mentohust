...
int switchState(int type)
{
	...
	if (sendCount>=MAX_SEND_COUNT && type!=ID_ECHO)  /* 超时太多次？ */
	{
		...
		{
		case ID_START:
			printf(_(">> Couldn't find the auth server,will restart auth!\n"));
			break;
		case ID_IDENTITY:
			printf(_(">> Timeout when send user name,will restart auth!\n"));
			break;
		case ID_CHALLENGE:
			printf(_(">> Timeout when send password,will restart auth!\n"));
			break;
		case ID_WAITECHO:
			printf(_(">> Timeout when wait for response.\n"));
...
{
	setTimer(0);	/* 取消定时器 */
	printf(_(">> Start to get IP...\n"));
	system(dhcpScript);
	printf(_(">> Finished operation.\n"));
...
static int sendEchoPacket()
{
	...
	if (sendCount == 0)
	{
		...
		printf(_(">> Send heartbeat package to keep alive...\n"));
